...
class FundraisingWithdrawalController extends Controller
{
    ...
    public function store(StoreFundraisingWithdrawalRequest $request, Fundraising $fundraising)
    {
        ...
        if ($hasRequestedWithdrawal) {
            return redirect()->route('admin.fundraisings.show', $fundraising->id)->with('error', 'Fundraising withdrawal request already sent');
        }
        ...
        DB::transaction(function () use ($request, $fundraising) {
            ...
            $validated['proof'] = 'proofs/default.png';

            $fundraising->withdrawals()->create($validated);
        });

        return redirect()->route('admin.my-withdrawals')->with('success', 'Fundraising withdrawal request sent successfully');
    }
    ...
    public function update(UpdateFundraisingWithdrawalRequest $request, FundraisingWithdrawal $fundraisingWithdrawal)
    {
        ...
        return redirect()->route('admin.fundraising_withdrawals.show', $fundraisingWithdrawal->id)->with('success', 'Fundraising withdrawal request updated successfully');
    }
