...
<section class="max-w-[640px] w-full min-h-screen mx-auto flex flex-col bg-white overflow-x-hidden">
        <div class="header flex flex-col bg-[#56BBC5] overflow-hidden h-[350px] relative -mb-[92px]">
            <nav class="pt-5 px-3 flex justify-between items-center relative z-20">
                <div class="flex items-center gap-[10px]">
                    <a href="{{ route('client.index') }}" class="w-10 h-10 flex shrink-0">
            ...
            <div class="w-full h-full absolute bg-white overflow-hidden">
                ...
                <img src="{{ asset('storage/'.$fundraising->thumbnail) }}" class="w-full h-full object-cover" alt="cover">
            </div>
        </div>
        <div class="flex flex-col z-30">
            ...
            <div id="content" class="w-full bg-white rounded-t-[40px] flex flex-col gap-5 p-[30px_24px_60px]">
                <div class="flex flex-col gap-[10px]">
                    ...
                    @else
                    <p class="badge bg-[#FF7815] rounded-full p-[6px_12px] font-bold text-xs text-white w-fit leading-[18px]">IN PROGRESS</p>
                    @endif
                    ...
                    <div class="flex items-center gap-2">
                        <div class="w-9 h-9 flex shrink-0 rounded-full overflow-hidden">
                            <img src="{{ asset('storage/'.$fundraising->fundraiser->user->avatar) }}" class="w-full h-full object-cover" alt="photo">
                        </div>
                ...
                @foreach($fundraising->fundraising_phases as $phase)
                <div class="flex flex-col gap-[10px] p-5 rounded-[20px] bg-[#F6ECE2]">
                    ...
                    <div class="aspect-[61/30] rounded-2xl bg-[#D9D9D9] overflow-hidden">
                        <img src="{{ asset('storage/'.$phase->photo) }}" class="w-full h-full object-cover" alt="thumbnail">
                    </div>
                ...
                <div class="flex flex-col gap-3">
                    ...
                    <div class="flex flex-col gap-4">
                        ...
                        <div class="flex items-center justify-center">
                            <p class="text-sm text-[#66697A]">No data</p>
                        </div>
                        @endforelse
                    </div>
                </div>
        ...
        @if(!$goalReached)
        <a href="{{ route('client.donation', $fundraising->slug) }}" class="p-[14px_20px] bg-[#76AE43] rounded-full text-white w-fit mx-auto font-semibold hover:shadow-[0_12px_20px_0_#76AE4380] transition-all duration-300 fixed bottom-[30px] transform -translate-x-1/2 left-1/2 z-40 text-nowrap">Send My Support Now</a>
        @endif
    </section>
@endsection