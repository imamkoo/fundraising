        ...
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-10 flex flex-col gap-y-5">
                ...
                <span class="text-white font-bold bg-red-500 rounded-2xl w-fit p-5">
                    <p class="text-center">
                        <span class="font-bold text-2xl flex flex-row justify-center items-center gap-x-2">
                            <i class="fa-solid fa-triangle-exclamation"></i>
                            <span class="text-md">
                                This fundraising is not active yet
                            </span>
                        </span>
                    </p>
                </span>
                @role('owner')
                    <form action="{{ route('admin.fundraising_withdrawals.activate_fundraising', $fundraising->id) }}" method="POST">
                            @csrf
                            <button type="submit" class="font-bold bg-green-700 text-white rounded-2xl w-fit p-5 hover:bg-green-800">
                                Approve
                            </button>
                    </form>
                    @endrole
                    ...
                    <span class="text-white font-bold bg-green-500 rounded-2xl w-fit p-5">
                        <p class="text-center">
                            <span class="font-bold text-2xl flex flex-row justify-center items-center gap-x-2">
                                <i class="fa-solid fa-triangle-exclamation"></i>
                                <span class="text-md">
                                    This fundraising is active now
                                 </span>
                            </span>
                        </p>
                    </span>
                ...
                <div class="item-card flex flex-row gap-y-10 justify-between items-center">
                    <div class="flex flex-row items-center gap-x-3">
                        <img src="{{ Storage::url($fundraising->thumbnail) }}" alt="" class="rounded-2xl object-cover w-[150px] h-[150px]">
                    ...
                    <div class="flex flex-row items-center gap-x-3">
                        <a href="{{ route('admin.fundraisings.edit', $fundraising->id) }}" class="font-bold py-4 px-6 bg-indigo-700 text-white rounded-full">
                            Edit
                        </a>
                        <form action="{{ route('admin.fundraisings.destroy', $fundraising->id) }}" method="POST">
                            @csrf
                            @method('DELETE')
                ...
                <form method="POST" action="{{ route('admin.fundraising_withdrawals.request', $fundraising->id) }}" enctype="multipart/form-data">
                    @csrf
                ...
                <div class="item-card flex flex-row gap-y-10 justify-between items-center">
                    ...
                    <p class="text-slate-500 text-sm">{{ $donatur->message }}</p>
                    
                </div>
                @empty
                <div class="flex flex-col items-center justify-center">
                    <h3 class="text-indigo-950 text-xl font-bold">No Donaturs</h3>
                    <p class="text-slate-500 text-base">No Donaturs found</p>
                </div>
                @endforelse
