    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-10 flex flex-col gap-y-5">
                ...
                @forelse ($fundraising_withdrawals as $fundraising_withdrawal)
                <div class="item-card flex flex-row justify-between items-center">
                    ...
                    <div>
                        ...
                        <h3 class="text-indigo-950 text-xl font-bold">Rp {{ number_format($fundraising_withdrawal->amount_requested, 0, ',', '.') }}</h3>
                    </div>
                    <div>
                        <p class="text-slate-500 text-sm">Date</p>
                        <h3 class="text-indigo-950 text-xl font-bold">{{ $fundraising_withdrawal->created_at }}</h3>
                    </div>
                    <div class="hidden md:flex flex-col">
                        <p class="text-slate-500 text-sm">Fundraiser</p>
                        <h3 class="text-indigo-950 text-xl font-bold">{{ $fundraising_withdrawal->fundraiser->user->name }}</h3>
                    </div>
                    @if ($fundraising_withdrawal->has_set)
                    @if ($fundraising_withdrawal->has_received)
                    ...
                    <div class="hidden md:flex flex-row items-center gap-x-3">
                        <a href="{{ route('admin.fundraising_withdrawals.show', $fundraising_withdrawal->id) }}" class="font-bold py-4 px-6 bg-indigo-700 text-white rounded-full">
                            View Details
                        </a>
                    </div>
                </div>
                @empty
                    <div class="item-card flex flex-col justify-between items-center">
                        <h3 class="text-indigo-950 text-xl font-bold">No Withdrawals Found</h3>
                    </div>
                @endforelse
            </div>
        </div>
