        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-10 flex flex-col gap-y-5">
                ...
                <div class="flex flex-row gap-x-16">
                    ...
                    <div>
                        <img src="{{ Storage::url($fundraisingWithdrawal->fundraising->thumbnail) }}" alt="" class="rounded-2xl object-cover w-[200px] h-[200px] mb-3">
                ...
                @if ($fundraisingWithdrawal->has_set)
                    ...
                    <hr class="my-5">
                    <h3 class="text-indigo-950 text-xl font-bold mb-5">Transfer Success</h3>
                    <img src="{{ Storage::url($fundraisingWithdrawal->proof) }}" alt="" class="rounded-2xl object-cover w-[300px] h-[200px] mb-3">
                    <hr class="my-5">

                    @if (!$fundraisingWithdrawal->fundraising->has_finished)
                    <h3 class="text-indigo-950 text-xl font-bold">Have You Delivered Money?</h3>
                    <form action="{{ route('admin.fundraising_phases.request', $fundraisingWithdrawal->fundraising_id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div>
                            <x-input-label for="name" :value="__('Testimoni')" />
                    ...
                    @else
                    <h3 class="text-indigo-950 text-xl font-bold">Fundraising has been finished</h3>
                    @endif
                @endif
            </div>
        </div>
