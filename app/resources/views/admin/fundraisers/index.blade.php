            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-10 flex flex-col gap-y-5">
                ...
                <div class="item-card flex flex-row justify-between items-center">
                    ...
                    @if ($fundraiser->is_active)
                        <span class="font-bold py-4 px-6 bg-green-500 text-white rounded-full">
                            ACTIVE
                        </span>
                    ...
                    @else
                        <span class="font-bold py-4 px-6 bg-red-500 text-white rounded-full">
                            PENDING
                        </span>
                        <div class="hidden md:flex flex-row items-center gap-x-3">
                        <form action="{{ route('admin.fundraisers.update', $fundraiser->id) }}" method="POST">
                            @csrf
                            @method('PUT')
                ...
                @empty
                    <div class="item-card flex flex-col justify-between items-center gap-y-5">
                        <h3 class="text-indigo-950 text-xl font-bold">No Fundraiser Found</h3>
                    </div>
                @endforelse
            </div>
        </div>
